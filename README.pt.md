...
3. **Otimização**: dadas as quantidades do seu inventário, um solver ILP encontra a combinação de poções de 2–3 ingredientes que maximiza o valor total em ouro. Veja [docs/calculation/CALCULATION.pt.md](docs/calculation/CALCULATION.pt.md) pra matemática exata, e [docs/data-sources/DATA_SOURCES.pt.md](docs/data-sources/DATA_SOURCES.pt.md) (mais [docs/game_data/GAME_DATA.pt.md](docs/game_data/GAME_DATA.pt.md) pra resolução de override) pra saber de onde vêm os dados de ingrediente/efeito.
...
Veja [docs/api/API.pt.md](docs/api/API.pt.md) pro formato completo de
entrada/saída de todo endpoint.
...
O CLI escolhe um backend de OCR automaticamente, nesta ordem:
...
Veja [docs/cache/plugin/PLUGIN_CACHE.pt.md](docs/cache/plugin/PLUGIN_CACHE.pt.md), [docs/cache/ingredients/INGREDIENTS_CACHE.pt.md](docs/cache/ingredients/INGREDIENTS_CACHE.pt.md), e [docs/cache/effects/EFFECTS_CACHE.pt.md](docs/cache/effects/EFFECTS_CACHE.pt.md) pra exatamente o que cada arquivo contém e como é populado.
...
Este projeto depende de duas bibliotecas de terceiros pouco usadas pra ler os próprios formatos binários de plugin/arquivo do Skyrim; veja [docs/dependencies/DEPENDENCIES.pt.md](docs/dependencies/DEPENDENCIES.pt.md) pro que foi verificado sobre elas e o risco de fixação de versão que decorre disso.
